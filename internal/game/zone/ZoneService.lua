...
local ZoneService <const> = Class.singleton('ZoneService', 'EventEmitter', function(class)
    ...
    function self:Register(zone)
        assert(not nox.is_server, 'ZoneService:Register is only available on client.');
    ...
    function self:Get(id)
        assert(not nox.is_server, 'ZoneService:Get is only available on client.');
        return self.zones[id];
    end

    ---@return table<string, InternalZone>
    function self:GetAll()
        assert(not nox.is_server, 'ZoneService:GetAll is only available on client.')
        return self.zones;
    end
    ...
    function self:GetByResource(resource, id)
        assert(not nox.is_server, 'ZoneService:GetByResource is only available on client.');
    ...
    function self:GetAllByResource(resource)
        assert(not nox.is_server, 'ZoneService:GetAllByResource is only available on client.');
        if (type(self.resources[resource]) == 'table') then
            local zones = {};
            for i = 1, #self.resources[resource] do
    ...
    function self:Remove(id)
        if (typeof(self.zones[id]) == 'InternalZone') then
            local zone, index = self:GetByResource(self.zones[id].resource, id);
            self.zones[id] = nil;
            self.resources[zone.resource][index] = nil;
    ...
    function self:RemoveByResource(resource)
        assert(not nox.is_server, 'ZoneService:RemoveByResource is only available on client.');
        if (type(self.resources[resource]) == 'table') then
            for i = 1, #self.resources[resource] do
