...
---@class noxen.lib.entity.player.net: EventEmitter
---@field public id number
---@field public identifier string
---@field public ped noxen.lib.entity.player_ped
---@overload fun(id: number): noxen.lib.entity.player.net
local Player = nox.class.extends('noxen.lib.entity.player.net', 'EventEmitter');
...
function Player:Constructor(id)
    ...
    self.ped = nox.entity.classes.player_ped(self.id);

    nox.entity.players[self.id] = self;
    PLAYERS_IDENTIFIERS[self.identifier] = self.id;

end

---@param id number | noxen.lib.entity.player.net
---@return noxen.lib.entity.player.net
function Player.Get(id)
    return is_instance(id) and id or nox.entity.players[tonumber(id)];
end
...
function Player.Remove(id)
    ...
    if (player) then
        nox.entity.players[player.id] = nil;
...
---@return noxen.lib.entity.player.net | nil
function Player.GetByIdentifier(identifier)
    return nox.entity.players[PLAYERS_IDENTIFIERS[identifier]];
end

---@param player? noxen.lib.entity.player.net
---@return boolean
function Player.IsValid(player)
    if (is_instance(player)) then
        ...
        if (typeof(ped) == 'noxen.lib.entity.player_ped') then
...
---@return noxen.lib.entity.player_ped
...
function Player:TriggerEvent(eventName, ...)
    nox.events.emit.net(eventName, self.id, ...);
    self:emit(eventName, ...);
end
...
function Player:TriggerCallback(eventName, callback, ...)
    nox.events.emit.callback(eventName, self.id, callback, ...);
end
...
function Player:ShowNotification(message, hudColorIndex, isTranslation, ...)
    nox.game.notification:SendTo(self.id, message, hudColorIndex, isTranslation, ...);
end
