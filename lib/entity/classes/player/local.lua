...
---@class noxen.lib.entity.player.local: EventEmitter
---@field public ped noxen.lib.entity.player_ped
---@overload fun(): noxen.lib.entity.player.local
local LocalPlayer = nox.class.extends("noxen.lib.entity.player.local", "EventEmitter");
...
function LocalPlayer:ShowNotification(message, hudColorIndex, isTranslation, ...)
    nox.game.notification:Send(message, hudColorIndex, isTranslation, ...);
end

---@param level number
function LocalPlayer:SetWantedLevel(level)
    assert(type(level) == "number", "noxen.lib.local_player:SetWantedLevellevel: must be a number");
    SET_PLAYER_WANTED_LEVEL(self.id, level, false);
end

---@param level number
function LocalPlayer:SetWantedLevelNow(level)
    assert(type(level) == "number", "noxen.lib.local_player:SetWantedLevelNow: must be a number");
