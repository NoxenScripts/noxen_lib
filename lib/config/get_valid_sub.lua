...
return function(parent, key)
    ...
    if (typeof(nox.config.default[parent]) == 'Config') then
        ...
        if (nox.config.default[parent].type == 'table') then
            ...
            local config = nox.config.get(parent);

            if (typeof(nox.config.default[parent].data[key]) 'Config') then
                if (type(config[key]) == nox.config.default[parent].data[key].type) then
                    return config[key];
                elseif (type(nox.config.default[parent].data[key].secondary_type) == 'string') then
                    if (type(config[key]) == nox.config.default[parent].data[key].secondary_type) then
                        return config[key];
                    end
                else
                    console.err(("^3Config key ^1Config.%s.%s^3 is not valid! Switching to default."):format(parent, key));
                    config[key] = nox.config.default[parent].data[key].value;
