...
---@param event noxen.lib.events.base_event
---@return boolean
function Event.Remove(event)
	if (is_instance(event)) then
		nox.events.remove(event.handler);
		events[event.id] = nil;
...
---@param callback fun(event: noxen.lib.events.base_event | noxen.lib.events.net_event, source: number, ...: any)
