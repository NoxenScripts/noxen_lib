...
UIMenu = nox.class.extends('UIMenu', 'EventEmitter');

local config_showHeader = nox.convar.get_value('noxen_lib_ui_show_header');
local config_displayGlare = nox.convar.get_value('noxen_lib_ui_show_glare');
local config_displaySubtitle = nox.convar.get_value('noxen_lib_ui_show_subtitle');
local config_displayBackground = nox.convar.get_value('noxen_lib_ui_show_background');
local config_displayNavigationBar = nox.convar.get_value('noxen_lib_ui_show_navbar');
local config_displayInstructionalButton = nox.convar.get_value('noxen_lib_ui_show_instructionnal_button');
local config_displayPageCounter = nox.convar.get_value('noxen_lib_ui_show_page_counter');
local config_titles = nox.convar.get_value('noxen_lib_ui_titles');
local config_titleFont = nox.convar.get_value('noxen_lib_ui_title_font');

local config_textureDict = nox.convar.get_value('noxen_lib_ui_texture_dictionnary');
local config_textureName = nox.convar.get_value('noxen_lib_ui_texture_name');
local config_color = {
    R = nox.convar.get_value('noxen_lib_ui_color_red'),
    G = nox.convar.get_value('noxen_lib_ui_color_green'),
    B = nox.convar.get_value('noxen_lib_ui_color_blue'),
    A = nox.convar.get_value('noxen_lib_ui_color_alpha')
};
...
function UIMenu:Constructor(Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
    ...
    self.DescriptionHeight = nox.ui.Settings.Items.Description.Background.Height
    self.X = X or 0
    self.Y = Y or 0
    self.Parent = nil
    self.WidthOffset = nox.ui.Data.Style[nox.ui.Data.Current].Width
    self.open = false
    self.Controls = nox.ui.Settings.Controls
    self.Index = 1
    self.Sprite = {
    ...
    self.CursorStyle = 1
    self.id = nox.uuid("xx6xx-xxxxx");
    self.data = {}

    if nox.ui.starts(self.Subtitle, "~") then
    ...
    if self.Subtitle ~= "" then
        local SubtitleLineCount = GetLineCount(self.Subtitle, self.X + nox.ui.Settings.Items.Subtitle.Text.X, self.Y + nox.ui.Settings.Items.Subtitle.Text.Y, 0, nox.ui.Settings.Items.Subtitle.Text.Scale, 245, 245, 245, 255, nil, false, false, nox.ui.Settings.Items.Subtitle.Background.Width + self.WidthOffset)
...
function UIMenu:IsVisible(Items, Panels, onClose)
    ...
    if type(Items) == "function" and ((Panels and type(Panels) == "function") or true) then
        ...
        self.func = type(Items) == "function" and Items or self.func;
        self.panels = type(Panels) == "function" and Panels or self.panels;
        self.Closed = type(onClose) == "function" and onClose or self.Closed;
    else
...
function UIMenu:GetSubMenus()
    if #SubMenus[self.id] > 0 then
        for _, submenu in pairs(SubMenus[self.id]) do
            nox.ui.IsVisible(Menus[submenu])
...
function UIMenu:Open()
    CreateThread(function()
        if (nox.ui.GetCurrentMenu() == nil and not IsPauseMenuActive()) then
            local Audio = nox.ui.Settings.Audio;
            nox.ui.PlaySound(Audio[Audio.Use].Select.audioName, Audio[Audio.Use].Select.audioRef);
            nox.ui.Visible(self, true);
            nox.ui.TextEnabled = false;
            CreateThread(function()
                while nox.ui.CurrentMenu ~= nil do
                    nox.ui.DisableControlsOnMenu();
                    nox.ui.IsVisible(self);
                    self:GetSubMenus();
                    Wait(1);
                end
            end);
...
function UIMenu:Close()
    local menu <const> = nox.ui.GetCurrentMenu();
    if (menu == self) then
        if (self.Closed) then
            self.Closed();
        end
        nox.ui.Visible(self, false);
        self.Index = 1
...
function UIMenu:Toggle()
    CreateThread(function()
        if (nox.ui.CurrentMenu ~= nil and nox.ui.CurrentMenu.Closable) then
            local Audio = nox.ui.Settings.Audio;
            nox.ui.PlaySound(Audio[Audio.Use].Back.audioName, Audio[Audio.Use].Back.audioRef);
            nox.ui.CurrentMenu:Close();
        else
            self:Open();
        end
    end);
...
function UIMenu:IsShowing()
    ...
    if (nox.ui.CurrentMenu == self) then
